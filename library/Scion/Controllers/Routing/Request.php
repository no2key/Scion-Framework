...
class Request {
	...
	public function clear($get = true, $post = true) {
		if ($get) {
			$_GET = array();
		}
		if ($post) {
	...
	public function isSecure() {
		if (is_null($this->isSecure)) {
			$this->isSecure = (isset($_SERVER['HTTPS']) && (strtolower($_SERVER['HTTPS']) == 'on' || $_SERVER['HTTPS'] == 1)) || (isset($_SERVER['HTTP_SSL_HTTPS']) && (strtolower($_SERVER['HTTP_SSL_HTTPS']) == 'on' || $_SERVER['HTTP_SSL_HTTPS'] == 1)) || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) == 'https');
		}
	...
	public function getHttpHeader($name, $prefix = 'HTTP') {
		...
		return isset($_SERVER[$name]) ? icUtil::stripslashes($_SERVER[$name]) : null;
	}
}