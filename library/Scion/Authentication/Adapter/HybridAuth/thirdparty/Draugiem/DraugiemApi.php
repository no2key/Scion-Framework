<?php
namespace Scion\Authentication\Adapter\HybridAuth\thirdparty\Draugiem;
...
class DraugiemApi {
	...
	public function getSession() {
		...
		elseif (isset($_GET['dr_auth_code']) && (empty($_SESSION['draugiem_auth_code']) || $_GET['dr_auth_code'] != $_SESSION['draugiem_auth_code'])) { // New session authorization
				...
				if (!empty($userData)) {
					if (!empty($_GET['session_hash'])) { //Internal application, store session key to recheck if draugiem.lv session is active
						...
						if (!empty($response['inviter'])) { //Fill invitation info if any
							$_SESSION['draugiem_invite'] = array('inviter' => (int)$response['inviter'],
	...
	public function getUserFriends($page = 1, $limit = 20, $return_ids = false) {
		$response = $this->apiCall('app_friends', array('show'  => ($return_ids ? 'ids' : false), 'page' => $page,
														'limit' => $limit
												  ));
		if ($response) {
	...
	public function getAllUserFriends($page = 1, $limit = 20, $return_ids = false) {
		$response = $this->apiCall('app_all_friends', array('show'  => ($return_ids ? 'ids' : false), 'page' => $page,
															'limit' => $limit
													  ));
		if ($response) {
			$this->lastTotal['friends'][$this->user_key] = (int)$response['total'];
	...
	public function getOnlineFriends($limit = 20, $in_app = false, $return_ids = false) {
		$response = $this->apiCall('app_friends_online', array('show'   => ($return_ids ? 'ids' : false),
															   'in_app' => $in_app, 'limit' => $limit
														 ));
		if ($response) {
			if ($return_ids) {
				return $response['userids'];
	...
	public function getAppUsers($page = 1, $limit = 20, $return_ids = false) {
		$response = $this->apiCall('app_users', array('show'  => ($return_ids ? 'ids' : false), 'page' => $page,
													  'limit' => $limit
												));
		if ($response) {
	...
	public function cookieFix() {
		...
		if (empty($_COOKIE[session_name()]) && strpos($user_agent, 'Safari') && isset($_GET['dr_auth_code']) && !isset($_GET['dr_cookie_fix'])) {
			?>
			<html>
			<head><title>Iframe Cookie fix</title></head>
			<body>
	...
	public function addNotification($text, $prefix = false, $link = false, $creator = 0) {
		$response = $this->apiCall('add_notification', array('text'    => $text, 'prefix' => $prefix, 'link' => $link,
															 'creator' => $creator
													   ));
		if (!empty($response['status'])) {
			if ($response['status'] == 'OK') {
	...
	private function clearSession() {
		unset($_SESSION['draugiem_auth_code'], $_SESSION['draugiem_session'], $_SESSION['draugiem_userkey'], $_SESSION['draugiem_user'], $_SESSION['draugiem_lastcheck'], $_SESSION['draugiem_language'], $_SESSION['draugiem_domain'], $_SESSION['draugiem_invite']);
	}
} 