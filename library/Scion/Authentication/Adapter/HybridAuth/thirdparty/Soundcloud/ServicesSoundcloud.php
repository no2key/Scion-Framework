...
class ServicesSoundcloud {
	 ...
	 * @static
	 */
	private static $_audioMimeTypes = array('aac' => 'video/mp4', 'aiff' => 'audio/x-aiff', 'flac' => 'audio/flac',
											'mp3' => 'audio/mpeg', 'ogg' => 'audio/ogg', 'wav' => 'audio/x-wav'
	);
	 ...
	 * @static
	 */
	private static $_curlDefaultOptions = array(CURLOPT_HEADER    => true, CURLOPT_RETURNTRANSFER => true,
												CURLOPT_USERAGENT => ''
	);
	 ...
	 * @static
	 */
	private static $_domains = array('development' => 'sandbox-soundcloud.com', 'production' => 'soundcloud.com'
	);
	 ...
	 * @static
	 */
	private static $_paths = array('authorize' => 'connect', 'access_token' => 'oauth2/token',
	);
	 ...
	 * @static
	 */
	private static $_responseFormats = array('*' => '*/*', 'json' => 'application/json', 'xml' => 'application/xml'
	);
	 ...
	 *
	 * @return void
	 * @throws ServicesSoundcloudMissingClientIdException
	 *
	 * @access public
	 */
	...
	function getAuthorizeUrl($params = array()) {
		$defaultParams = array('client_id'     => $this->_clientId, 'redirect_uri' => $this->_redirectUri,
							   'response_type' => 'code'
		);
	...
	function credentialsFlow($username, $password) {
		$postData = array('client_id' => $this->_clientId, 'client_secret' => $this->_clientSecret,
	...
	function accessToken($code = null, $postData = array(), $curlOptions = array()) {
		$defaultPostData = array('code'          => $code, 'client_id' => $this->_clientId,
								 'client_secret' => $this->_clientSecret, 'redirect_uri' => $this->_redirectUri,
	...
	function accessTokenRefresh($refreshToken, $postData = array(), $curlOptions = array()) {
		$defaultPostData = array('refresh_token' => $refreshToken, 'client_id' => $this->_clientId,
								 'client_secret' => $this->_clientSecret, 'redirect_uri' => $this->_redirectUri,
	 ...
	 *
	 * @return string
	 * @throws ServicesSoundcloudUnsupportedAudioFormatException
	 *
	 * @access public
	 */
	 ...
	 *
	 * @return object
	 * @throws ServicesSoundcloudUnsupportedResponseFormatException
	 *
	 * @access public
	 */
	...
	function put($path, $postData, $curlOptions = array()) {
		$url     = $this->_buildUrl($path);
		$options = array(CURLOPT_CUSTOMREQUEST => 'PUT', CURLOPT_POSTFIELDS => $postData
		);
		$options += $curlOptions;
	...
	function download($trackId, $params = array(), $curlOptions = array()) {
		...
		$defaultCurlOptions = array(CURLOPT_FOLLOWLOCATION => true, self::CURLOPT_OAUTH_TOKEN => false
		);
		$url                = $this->_buildUrl('tracks/' . $trackId . '/download', array_merge($defaultParams, $params));
	...
	public function updatePlaylist($playlistId, $trackIds, $optionalPostData = null) {
		...
		$curlOptions = array(CURLOPT_CUSTOMREQUEST => 'PUT',
	 ...
	 *
	 * @return mixed
	 * @throws ServicesSoundcloudInvalidHttpResponseCodeException
	 *
	 * @access protected
	 */
	protected function _request($url, $curlOptions = array()) {
		...
		if (array_key_exists(CURLOPT_HTTPHEADER, $options)) {
			$options[CURLOPT_HTTPHEADER] = array_merge($this->_buildDefaultHeaders(), $curlOptions[CURLOPT_HTTPHEADER]);
		}
		else {
		...
		if (array_key_exists(CURLOPT_HEADER, $options) && $options[CURLOPT_HEADER]) {
			$this->_lastHttpResponseHeaders = $this->_parseHttpHeaders(substr($data, 0, $info['header_size']));
		...
		if ($this->_validResponseCode($this->_lastHttpResponseCode)) {
			...
			throw new ServicesSoundcloudInvalidHttpResponseCodeException(null, 0, $this->_lastHttpResponseBody, $this->_lastHttpResponseCode);
		}
	}
