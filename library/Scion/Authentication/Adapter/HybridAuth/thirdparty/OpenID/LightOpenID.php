<?php
namespace Scion\Authentication\Adapter\HybridAuth\thirdparty\OpenID;
...
class LightOpenID {
	...
	static protected $ax_to_sreg = array('namePerson/friendly'  => 'nickname', 'contact/email' => 'email',
										 'namePerson'           => 'fullname', 'birthDate' => 'dob',
										 'person/gender'        => 'gender', 'contact/postalCode/home' => 'postcode',
										 'contact/country/home' => 'country', 'pref/language' => 'language',
	...
	protected function request_streams($url, $method = 'GET', $params = array(), $update_claimed_id) {
		...
		switch ($method) {
			case 'GET':
				$opts = array('http' => array('method'        => 'GET', 'header' => 'Accept: application/xrds+xml, */*',
											  'ignore_errors' => true,
			...
			case 'POST':
				$opts = array('http' => array('method'  => 'POST',
											  ...
											  'content' => $params, 'ignore_errors' => true,
				), 'ssl'             => array('CN_match' => parse_url($url, PHP_URL_HOST),
				),
				);
				if (!empty($this->proxy)) {
	...
	protected function authUrl_v1($immediate) {
		...
		$params = array('openid.return_to' => $returnUrl,
						...
						'openid.identity'  => $this->identity, 'openid.trust_root' => $this->trustRoot,
			) + $this->sregParams();
	...
	protected function authUrl_v2($immediate) {
		$params = array('openid.ns'        => 'http://specs.openid.net/auth/2.0',
						'openid.mode'      => $immediate ? 'checkid_immediate' : 'checkid_setup',
						'openid.return_to' => $this->returnUrl, 'openid.realm' => $this->trustRoot,
		);
	...
	function validate() {
		...
		$params           = array('openid.assoc_handle' => $this->data['openid_assoc_handle'],
