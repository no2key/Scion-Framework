...
use Scion\Authentication\Adapter\HybridAuth\thirdparty\Vimeo\VimeoAPIException;

/**
 * Hybrid_Providers_Vimeo class, wrapper for Vimeo
 */
class Vimeo extends ProviderModel {
	/**
	...
	function initialize() {
		...
		require_once Auth::$config["path_libraries"] . "Vimeo/Vimeo.php";

		if ($this->token("access_token") && $this->token("access_token_secret")) {
			$this->api = new \Scion\Authentication\Adapter\HybridAuth\thirdparty\Vimeo\Vimeo
	 ...
	 */
	function loginBegin() {
		$this->api = new \Scion\Authentication\Adapter\HybridAuth\thirdparty\Vimeo\Vimeo($this->config["keys"]["key"], $this->config["keys"]["secret"]);
		...
		Auth::redirect($this->api->getAuthorizeUrl($tokz['oauth_token'], 'write'));
	}
	...
	function loginFinish() {
		...
		try {
			$this->api = new \Scion\Authentication\Adapter\HybridAuth\thirdparty\Vimeo\Vimeo($this->config["keys"]["key"], $this->config["keys"]["secret"], $this->token("request_token"), $this->token("request_token_secret"));
