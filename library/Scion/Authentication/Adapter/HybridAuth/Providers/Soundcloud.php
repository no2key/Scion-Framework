...
use Scion\Authentication\Adapter\HybridAuth\thirdparty\Soundcloud\ServicesSoundcloud;

class Soundcloud extends ProviderModel {
	/**
	...
	function initialize() {
		...
		require_once Auth::$config["path_libraries"] . "Soundcloud/Soundcloud.php";

		$this->api = new ServicesSoundcloud($this->config["keys"]["key"], $this->config["keys"]["secret"], $this->endpoint);
	...
	function loginBegin() {
		...
		Auth::redirect($this->api->getAuthorizeUrl());
	}
